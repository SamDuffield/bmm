...
from inference.resampling import fixed_lag_stitching, multinomial, fixed_lag_stitch_post_split
from inference.backward import backward_simulate
...
def update_particles_flbs(graph,
    ...
    # Propose and weight for each particle
    for j in range(n):
        latest_filter_particles[j], weights[j] = proposal(graph, base_particles[j], new_observation,
